...
```python
        ...
        self.txtInput.setFixedHeight(50)
        root.addWidget(self.txtInput)

        # ===== Таблица модели =====
        self.modelBox = QGroupBox("Таблица символов (символ, вероятность, нижняя и верхняя границы)")
        model_layout = QVBoxLayout(self.modelBox)
        self.modelTable = ModelTable()
        model_layout.addWidget(self.modelTable)
        root.addWidget(self.modelBox)

        # ===== Таблица шагов =====
        table_box = QGroupBox("Таблица шагов (Шаг, цепочка, интервал)")
        table_layout = QVBoxLayout(table_box)
        self.tblSteps = StepsTable()
        table_layout.addWidget(self.tblSteps)
        ...
        result_layout.addWidget(self.resultView, 1)
